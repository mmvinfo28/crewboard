...
export function GET(request: NextRequest) {
  ...
  const script = [
    ...
    ...(startupFlag ? ["  npx --yes github:mmvinfo28/crewboard#connector-v0.4.2 startup on"] : []),
    `  npx --yes github:mmvinfo28/crewboard#connector-v0.4.2 restart --url "${serverUrl}"`,
    ") else (",
    `  npx --yes github:mmvinfo28/crewboard#connector-v0.4.2 connect --url "${serverUrl}" --background${startupFlag}`,
    ")",
    "echo.",
